...
function AbilityButton({ability, callBackFunction, imgSource, isActive}: {ability: ChampionPassive | ChampionSpell, callBackFunction: Function, imgSource: string, isActive: boolean}) {
  return (
    <button onClick={() => callBackFunction(ability)} className={clsx(["relative group"])}>
      <div className={clsx(["absolute top-0 left-0 w-full h-full bg-background opacity-60 transition-opacity group-hover:opacity-0", isActive && "opacity-0"])}></div>
      <Image src={imgSource}
        isStatic={false}  
        loadingType="lazy" 
        width={64} 
...
function AbilityDescription({ability, isActiveItem}: {ability: ChampionPassive | ChampionSpell, isActiveItem: boolean}){
  ...
  return (
    <div className={clsx(["absolute top-0 left-0 w-full"], isActiveItem ? "opacity-100" : "opacity-0")}>
      <div className="relative">
        <h3 className="pl-4 mb-4 overflow-hidden text-2xl text-medium font-bebas">
          <span className={clsx(["inline-block transition-transform translate-y-full underline underline-offset-2", isActiveItem ? "translate-y-0" : "translate-y-full"])}>{ability.name}</span>
        </h3>
        <p className={clsx(["relative pb-4 pl-4 transition-opacity delay-100 duration-300 leading-tight",isActiveItem ? "opacity-100" : "opacity-0"])}>
    ...
    <div>
      ...
      <div className="flex flex-col gap-8">
        <div className="relative flex gap-4">
          ...
          />
          {Array.isArray(abilities) && abilities.map(ability => <AbilityButton 
            key={ability.id}
            callBackFunction={setActiveItem} 
            ability={ability}
        ...
        <div className="grid gap-2 justify-items-start">
          ...
          <ol className="relative pb-[300px] w-full max-w-prose md:pb-[250px] lg:pb-0">
            {arrayOfAllAbilities.map(ability => <AbilityDescription ability={ability} isActiveItem={activeItem === ability} key={ability.name} />)}
          </ol>
        </div>
      </div>
    </div>
